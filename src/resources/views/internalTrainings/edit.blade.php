...
<div class="card">
    <div class="card-header">
        {{ trans('pqeAdmin::global.edit') }} {{ trans('pqeAdmin::cruds.internal_trainings.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("internal-trainings.update", [$internals->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group col-6">
                <label class="required" for="training_name">{{ trans('pqeAdmin::cruds.internal_trainings.fields.training_name') }}</label>
                <input class="form-control {{ $errors->has('training_name') ? 'is-invalid' : '' }}" type="text" name="training_name" id="training_name" value="{{ old('training_name', $internals->training_name) }}" required>
            ...
            <div class="form-group col-6">
                ...
                <input class="form-control {{ $errors->has('training_duration') ? 'is-invalid' : '' }}" type="text" name="training_duration" id="training_duration" value="{{ old('training_duration',number_format($internals->training_duration ?? 0, 2, '.', '')) }}" required>
