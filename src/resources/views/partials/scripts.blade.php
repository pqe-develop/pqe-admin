    ...
    <script>
        $(function() {
  let copyButtonTrans = '{{ trans('pqeAdmin::global.datatables.copy') }}'
  let csvButtonTrans = '{{ trans('pqeAdmin::global.datatables.csv') }}'
  let excelButtonTrans = '{{ trans('pqeAdmin::global.datatables.excel') }}'
  let pdfButtonTrans = '{{ trans('pqeAdmin::global.datatables.pdf') }}'
  let printButtonTrans = '{{ trans('pqeAdmin::global.datatables.print') }}'
  let colvisButtonTrans = '{{ trans('pqeAdmin::global.datatables.colvis') }}'
  let selectAllButtonTrans = '{{ trans('pqeAdmin::global.select_all') }}'
  let selectNoneButtonTrans = '{{ trans('pqeAdmin::global.deselect_all') }}'
  ...
  /*    {
      ...
      },*/
      {
        ...
        exportOptions: {
          columns: 'th:not(.sorting_disabled)'
         }
      },
  /*  {
        extend: 'pdf',
  ...
  $.fn.dataTable.ext.classes.sPageButton = '';
        ...
        if (!$(this).hasClass('open')) {
             ...
             $.ajax({
                ...
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    ...
                    console.log(false, 'Status: ' + textStatus + ' Error: ' + errorThrown);
                }
            });
    ...
    $('.searchable-field').select2({
        ...
        placeholder : '{{ trans('pqeAdmin::global.search') }}...',
        language: {
            inputTooShort: function(args) {
                var remainingChars = args.minimum - args.input.length;
                var translation = '{{ trans('pqeAdmin::global.search_input_too_short') }}';

                return translation.replace(':count', remainingChars);
            },
            errorLoading: function() {
                return '{{ trans('pqeAdmin::global.results_could_not_be_loaded') }}';
            },
            searching: function() {
                return '{{ trans('pqeAdmin::global.searching') }}';
            },
            noResults: function() {
                return '{{ trans('pqeAdmin::global.no_results') }}';
            },
        }
    ...
    function formatItemSelection (item) {
        if (!item.model) {
            return '{{ trans('pqeAdmin::global.search') }}...';
        }
        return item.model;
    }
    ...
    <!-- //Tooltip changes for help-block -->
    <script>
